...
header('Content-Type: text/html; charset=UTF-8');
  ...
  if ($errors['name']) {
    ...
    $messages['name_message'] = '<div class="error">Введите ваше имя, пожалуйста.<br>Поле может быть заполнено символами только русского или только английского алфавитов</div>';
  }
  if ($errors['email']) {
    setcookie('email_error', '', 100000);
    $messages['email_message'] = '<div class="error">Введите ваш e-mail, пожалуйста.<br>Поле может быть заполнено только символами английского алфавита, цифрами и знаком "@"</div>';
  }
  if ($errors['year']) {
    setcookie('year_error', '', 100000);
    $messages['year_message'] = '<div class="error">Выберите ваш год рождения, пожалуйста</div>';
  }
  if ($errors['gender']) {
    setcookie('gender_error', '', 100000);
    $messages['gender_message'] = '<div class="error">Укажите ваш пол, пожалуйста</div>';
  }
  if ($errors['kon']) {
    setcookie('kon_error', '', 100000);
    $messages['kon_message'] = '<div class="error">Веберите кол-во ваших конечностей</div>';
  }
  if ($errors['super']) {
    setcookie('super_error', '', 100000);
    $messages[] = '<div class="error">Ваши сверхспособности</div>';
  }
  if ($errors['bio']) {
    setcookie('bio_error', '', 100000);
    $messages['bio_message'] = '<div class="error">Расскажите, пожалуйста, о себе</div>';
  }
  if ($errors['contr_check']) {
    setcookie('contr_check_error', '', 100000);
    $messages['contr_check_message'] = '<div class="error">Ознакомьтесь с контрактом, для отправки</div>';
  }
